...
trigger UpdateStockWhenNeedlesExchanged on Needle_Exchange__c (after insert, after update) 
{
	...
	set <String> stockLocations = new set <String>();
	for (Needle_Exchange__c oneExchange: Trigger.New)
	{
		stockLocations.add (oneExchange.Location__c);
	}
	list <Needle_Syringe_Stock__c> locationsList = [Select Name, X1ml_Diabetic__c, X1ml_Filter__c, X1ml_Nevershare__c, X1ml_Omnifix__c, X2ml_Green__c, X2ml_Never_Share_Syringe__c,
	             ...
	             from Needle_Syringe_Stock__c where Name in :stockLocations];
	             
	map <String, Needle_Syringe_Stock__c> locationsMap = new map <String, Needle_Syringe_Stock__c>();
	for (Needle_Syringe_Stock__c oneLocation: locationsList)
	{
		locationsMap.put (oneLocation.Name, oneLocation);
	}
    ...
    for (Needle_Exchange__c oneExchange: Trigger.New)
    {
        try
        {
            ...
            Needle_Syringe_Stock__c stock = locationsMap.get (location);
